...
const Swatch = ({ name, hex, cssVar, textDark = false }: SwatchProps) => (
  <div style={{ display: "flex", flexDirection: "column", gap: 0, overflow: "hidden", borderRadius: "10px", border: "1px solid rgba(255,255,255,0.06)", minWidth: "120px", flex: "1 1 100px", maxWidth: "160px" }}>
    <div style={{ background: hex, height: "64px" }} />
    <div style={{ padding: "10px 12px", background: "#1E1E1E" }}>
      <p style={{ color: "#F6F9FC", fontFamily: "Inter, sans-serif", fontSize: "12px", fontWeight: 500, margin: 0 }}>{name}</p>
      <p style={{ color: "#5F6B7A", fontFamily: "JetBrains Mono, monospace", fontSize: "11px", margin: "2px 0 0" }}>{hex}</p>
      {cssVar && <p style={{ color: "#3A3A3A", fontFamily: "JetBrains Mono, monospace", fontSize: "10px", margin: "1px 0 0" }}>{cssVar}</p>}
    </div>
  </div>
);
...
const Section = ({ title, subtitle, children }: { title: string; subtitle?: string; children: React.ReactNode }) => (
  <div style={{ marginBottom: "2.5rem" }}>
    <div style={{ marginBottom: "16px" }}>
      <p style={{ color: "#F6F9FC", fontFamily: "Inter, sans-serif", fontSize: "16px", fontWeight: 600, margin: 0 }}>{title}</p>
      {subtitle && <p style={{ color: "#5F6B7A", fontFamily: "Inter, sans-serif", fontSize: "13px", margin: "4px 0 0" }}>{subtitle}</p>}
    </div>
...
const SemanticRow = ({ token, value, desc }: { token: string; value: string; desc: string }) => (
  <div style={{ display: "flex", alignItems: "center", gap: "16px", padding: "10px 0", borderBottom: "1px solid #2A2A2A" }}>
    <div style={{ width: "20px", height: "20px", borderRadius: "4px", background: value, border: "1px solid rgba(255,255,255,0.08)", flexShrink: 0 }} />
    <code style={{ color: "#00D4B8", fontFamily: "JetBrains Mono, monospace", fontSize: "12px", flexShrink: 0, minWidth: "200px" }}>{token}</code>
    <code style={{ color: "#5F6B7A", fontFamily: "JetBrains Mono, monospace", fontSize: "11px", flexShrink: 0, minWidth: "80px" }}>{value}</code>
    <p style={{ color: "#98A7B5", fontFamily: "Inter, sans-serif", fontSize: "12px", margin: 0 }}>{desc}</p>
  </div>
);
  ...
  <div style={{ maxWidth: "900px", padding: "2rem", fontFamily: "Inter, sans-serif" }}>
    ...
    <div style={{ marginBottom: "2.5rem" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "8px" }}>
        <div style={{ width: "32px", height: "32px", borderRadius: "8px", background: "linear-gradient(135deg, #0A2540, #00D4B8)" }} />
        <h1 style={{ color: "#F6F9FC", fontSize: "24px", fontWeight: 700, margin: 0 }}>Paleta de Cores</h1>
      </div>
    ...
    {/* Navy Scale */}
    <Section title="Navy — Primary (60%)" subtitle="Cor principal da marca. Use em backgrounds, headers e elementos de estrutura.">
    ...
    {/* Mint Scale */}
    <Section title="Mint — CTA (10%)" subtitle="Cor de destaque. Reserve para o único CTA por tela, badges de status ao vivo e indicadores ativos.">
    ...
    {/* Neutral Scale */}
    <Section title="Neutral — Supporting (30%)" subtitle="Escala de cinzas para texto, bordas, superfícies e ícones. Base funcional da interface.">
    ...
    {/* Status Colors */}
    <Section title="Status" subtitle="Feedback semântico para validações, alertas e estados de erro.">
    ...
    <div>
      <div style={{ marginBottom: "16px" }}>
        <p style={{ color: "#F6F9FC", fontFamily: "Inter, sans-serif", fontSize: "16px", fontWeight: 600, margin: 0 }}>Tokens Semânticos</p>
        <p style={{ color: "#5F6B7A", fontFamily: "Inter, sans-serif", fontSize: "13px", margin: "4px 0 0" }}>Variáveis prontas para uso nos componentes. Nunca use cores brutas — use sempre os tokens.</p>
      </div>
      ...
      <SemanticRow token="--cta" value="#00D4B8" desc="Botão CTA, badges de destaque, switch ativo" />
      ...
      <SemanticRow token="--muted-foreground" value="#5F6B7A" desc="Texto secundário, placeholders, ícones" />
