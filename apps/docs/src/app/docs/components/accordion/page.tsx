        ...
        >
          <Accordion type="single" collapsible className="w-full">
            ...
            <AccordionItem value="item-2">
              ...
              <AccordionContent>
                Os pacotes base são <code style={{ fontFamily: "var(--font-mono)" }}>@codeforward/tokens</code>,{" "}
...
<Accordion type="multiple">
  ...
  <AccordionItem value="item-2">...</AccordionItem>
      ...
      <section className="space-y-4">
        <h2 className="text-xl font-medium" style={{ color: "var(--foreground)" }}>Props principais</h2>
        <div className="overflow-x-auto rounded-xl border" style={{ borderColor: "var(--border)" }}>
          <table className="w-full text-sm">
            ...
            <tbody>
              {[
                ["Accordion", "type", '"single" | "multiple"', 'single: um item aberto por vez. multiple: vários simultâneos'],
                ["Accordion", "collapsible", "boolean", "Permite fechar o item aberto (somente type=single)"],
                ["Accordion", "defaultValue", "string | string[]", "Item(s) aberto(s) por padrão (não controlado)"],
                ["AccordionItem", "value", "string *", "Identificador único do item"],
              ].map(([comp, prop, type, desc], i) => (
                <tr key={`${comp}-${prop}`} style={{ backgroundColor: i % 2 === 0 ? "var(--background)" : "var(--muted)" }}>
                  ...
                  <td className="px-4 py-3 border-b" style={{ borderColor: "var(--border)" }}>
                    <code style={{ fontFamily: "var(--font-mono)", fontSize: "0.8rem" }}>{prop}</code>
                  </td>
                  <td className="px-4 py-3 border-b" style={{ borderColor: "var(--border)" }}>
                    <code style={{ fontFamily: "var(--font-mono)", fontSize: "0.8rem", color: "var(--color-mint-600)" }}>{type}</code>
                  </td>
