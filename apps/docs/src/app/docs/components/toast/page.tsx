...
const props = [
  { name: "variant", type: '"default" | "success" | "warning" | "destructive"', default: '"default"', description: "Estilo visual — cada variante exibe um ícone automático" },
  { name: "open", type: "boolean", description: "Estado controlado de visibilidade" },
  { name: "onOpenChange", type: "(open: boolean) => void", description: "Callback ao mudar visibilidade" },
  { name: "duration", type: "number", default: "5000", description: "Tempo em ms antes de fechar automaticamente" },
];
  ...
  return (
    <div className="space-y-10" style={{ fontFamily: "var(--font-sans)" }}>
      ...
      <section className="space-y-4">
        ...
        <CodeBlock language="bash" code="npm install @radix-ui/react-toast class-variance-authority" />
      </section>

      <hr style={{ borderColor: "var(--border)" }} />

      <section className="space-y-4">
        <h2 className="text-xl font-medium" style={{ color: "var(--foreground)" }}>Setup</h2>
...
export default function RootLayout({ children }) {
  return (
    <html>
      ...
      <section className="space-y-4">
        <h2 className="text-xl font-medium" style={{ color: "var(--foreground)" }}>Uso</h2>
        <CodeBlock
          language="tsx"
...
function MyComponent() {
  ...
  return (
    <>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {[
                ...
                <tr key={variant as string} style={{ backgroundColor: i % 2 === 0 ? "var(--background)" : "var(--muted)" }}>
                  <td className="px-4 py-3 border-b" style={{ borderColor: "var(--border)" }}>
                    <code style={{ fontFamily: "var(--font-mono)", color: "var(--primary)" }}>{variant}</code>
                  </td>
                  <td className="px-4 py-3 border-b" style={{ borderColor: "var(--border)", color: "var(--muted-foreground)" }}>{icon}</td>
                  <td className="px-4 py-3 border-b" style={{ borderColor: "var(--border)", color: "var(--foreground)" }}>{use}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section className="space-y-4">
        <h2 className="text-xl font-medium" style={{ color: "var(--foreground)" }}>Props — Toast</h2>
